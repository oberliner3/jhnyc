...
export function ChatWidget({
  ...
  // Experience tracking
  const trackChatOpen = useClickTracking('chat-widget-open');
  const trackChatClose = useClickTracking('chat-widget-close');
  const trackChatMinimize = useClickTracking('chat-widget-minimize');
  const trackChatMaximize = useClickTracking('chat-widget-maximize');
  ...
  const getButtonStyles = () => {
    const baseStyles = "relative shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105";
  ...
  const getChatWindowStyles = () => {
    const baseStyles = "border-none rounded-2xl shadow-2xl transition-all duration-300 ease-in-out";
    ...
    <div className={getPositionClasses()}>
      ...
      {isOpen && (
        <div 
          className={`
            ${getChatWindowStyles()}
            ${isMinimized ? 'h-12' : `h-[${typeof height === 'number' ? height + 'px' : height}]`}
            w-[${typeof width === 'number' ? width + 'px' : width}]
            max-w-[90vw] max-h-[80vh]
            overflow-hidden
          `}
          style={{
            height: isMinimized ? '48px' : typeof height === 'number' ? `${height}px` : height,
          ...
          {!isMinimized && (
            <div className="relative flex-1 overflow-hidden">
              ...
              <iframe
                ...
                style={{
                  display: 'block',
                  height: `${typeof height === 'number' ? height - 48 : 'calc(100% - 48px)'}`,
                }}
