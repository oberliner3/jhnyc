...
CREATE TABLE order_items (
  ...
  variant_id UUID, -- Assuming variant IDs are UUIDs or can be null
...
CREATE POLICY "Users can insert cart items for their cart." ON cart_items
  FOR INSERT
  WITH CHECK (EXISTS (SELECT 1 FROM carts WHERE carts.id = cart_id AND carts.user_id = auth.uid()));

CREATE POLICY "Users can update cart items for their cart." ON cart_items
  FOR UPDATE
  USING (EXISTS (SELECT 1 FROM carts WHERE carts.id = cart_id AND carts.user_id = auth.uid()));

CREATE POLICY "Users can delete cart items from their cart." ON cart_items
  FOR DELETE
  USING (EXISTS (SELECT 1 FROM carts WHERE carts.id = cart_id AND carts.user_id = auth.uid()));