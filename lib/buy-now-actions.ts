...
import type { ApiProductVariant } from "@/lib/types";
...
export async function buyNowAction(formData: FormData): Promise<string> {
  try {
    ...
    if (!Number.isFinite(quantity) || quantity < LIMITS.MIN_QUANTITY_PER_ITEM || quantity > LIMITS.MAX_QUANTITY_PER_ITEM) {
      throw new Error(`Quantity must be between ${LIMITS.MIN_QUANTITY_PER_ITEM} and ${LIMITS.MAX_QUANTITY_PER_ITEM}`);
    }
    ...
    if (!response.ok) {
      ...
      try {
        ...
        errorMessage = `API returned non-JSON response: ${responseText.substring(0, 200)}`;
      }
      logError(new Error(errorMessage), { context: "buyNowAction - API Error", payload: shopifyPayload });
    ...
    try {
      ...
      logError(parseError instanceof Error ? parseError : new Error("Invalid JSON"), { context: "buyNowAction - Parse JSON" });
    ...
    logError(error instanceof Error ? error : new Error("Unknown error"), { context: "buyNowAction", formData: Object.fromEntries(formData.entries()) });
    throw new Error(error instanceof Error ? error.message : ERROR_MESSAGES.SERVER_ERROR);
  }
}
