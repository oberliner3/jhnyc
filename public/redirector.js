...
(function() {
  ...
  // 1. Check if current domain is legitimate
  ...
  function hasValidProxyToken() {
    ...
    // This should match your PROXY_SECRET_TOKEN
  ...
  function protectDomain() {
    ...
    if (!inIframe && !legitimateHost) {
      console.warn('[Domain Protection] Unauthorized domain detected, redirecting...');
      window.location.replace('https://' + LEGITIMATE_DOMAIN + window.location.pathname);
      return;
    }

    // Case 2: In iframe but parent is not authorized
    if (inIframe && !legitimateParent && !validToken) {
      console.warn('[Domain Protection] Unauthorized iframe parent, breaking out...');
    ...
    if (inIframe && (legitimateParent || validToken)) {
      ...
      window.addEventListener('message', function(event) {
        ...
        if (!ALLOWED_DOMAINS.some(domain => event.origin.includes(domain))) {
          console.warn('[Domain Protection] Ignored message from unauthorized origin:', event.origin);
          return;
        }
        
        console.log('[Domain Protection] Received message from parent:', event.data);
      });
    }
  }
      ...
      mutations.forEach(function(mutation) {
        mutation.removedNodes.forEach(function(node) {
          if (node.id === 'domain-redirect') {
            console.error('[Domain Protection] Protection script removed! Re-running checks...');
            protectDomain();
          }
        });
      });
