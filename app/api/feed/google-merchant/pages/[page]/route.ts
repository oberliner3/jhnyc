...
export async function GET(
  ...
  try {
    logger.info(`Generating Google Merchant feed page ${page}`);

    // Fetch all products
    const allProducts = await fetchAllProducts();
    
    if (!allProducts || allProducts.length === 0) {
      logger.warn("No products found for Google Merchant feed");
    ...
    const xml = generateMerchantFeedXml(
      ...
      SITE_CONFIG.name,
      SITE_CONFIG.url,
      `Product feed for Google Merchant Center - Page ${page} of ${totalPages}`,
      feedType
    );

    logger.info(
      `Generated Google Merchant feed page ${page} with ${allItems.length} items (${errorCount} errors)`
    );
    ...
    logger.error(`Error generating Google Merchant feed page ${page}`, error);
