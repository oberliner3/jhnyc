  ...
  try {
    ...
    for (const item of lineItems) {
      if (!item.quantity || item.quantity < 1) {
        return NextResponse.json(
          { success: false, error: "Each line item must have a valid quantity" },
          { status: 400 }
        );
      }
    }
    ...
    if (sendInvoice && draftOrder.id && customerEmail) {
      try {
        await sendDraftOrderInvoice(draftOrder.id, {
          to: customerEmail,
          subject: invoiceData?.subject || `Invoice #${draftOrder.name || draftOrder.id}`,
        ...
        invoiceError = error instanceof Error ? error.message : "Failed to send invoice";
      }
    }
...
export async function GET() {
  return NextResponse.json(
    {
      success: false,
      error: "This endpoint only accepts POST requests. Use POST to create draft orders.",
    },
    { status: 405 }
  );
}