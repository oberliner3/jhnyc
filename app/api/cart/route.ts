...
export async function GET(): Promise<NextResponse<CartWithItems | { id: null; cart_items: [] } | { message: string }>> {
  try {
...
export async function POST(): Promise<NextResponse<CartWithItems | { message: string }>> {
  try {
    ...
    if (existingCart) {
      console.log(`[CART] User ${user.id} already has a cart:`, existingCart.id);
      return NextResponse.json(
